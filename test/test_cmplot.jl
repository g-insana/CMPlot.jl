...
#test_cmplot_traces_generation
println("Testing cmplot_traces_generation")
traces, layout = cmplot(df, xcol=:Species,
        ycol=[:SepalLength, :PetalLength],
        colorrange=3,
...
@test isa(traces, Array{GenericTrace, 1})
@test isa(layout, Layout)
...
@test_throws DomainError cmplot(df, xcol=:Species, ycol=[:Sepallll, :PetalLength])
